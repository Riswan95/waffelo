    ...
    TextureManager *textureM)
{
	...
	bubble.initialize(gamePtr->getGraphics(), width, height, ncols, textureM);
	bubble.setFrames(cakeNS::BUBBLE_START_FRAME, cakeNS::BUBBLE_START_FRAME);
	bubble.setCurrentFrame(cakeNS::BUBBLE_START_FRAME);
	bubble.setFrameDelay(cakeNS::BUBBLE_ANIMATION_DELAY);
	bubble.setLoop(true);                  // loop animation
...
void Cake::draw()
{
	...
	else if (waffleBubble == false)
	{
		bubble.setAnimationComplete(false);
	}
}
...
void Cake::update(float frameTime)
{
    ...
    //Bounce off walls
	if (spriteData.x > GAME_WIDTH - cakeNS::WIDTH * getScale())    // if hit right screen edge
    {
		spriteData.x = GAME_WIDTH - cakeNS::WIDTH * getScale();    // position at right screen edge
	...
	if (spriteData.y > GAME_HEIGHT - cakeNS::HEIGHT * getScale())  // if hit bottom screen edge
    {
		spriteData.y = GAME_HEIGHT - cakeNS::HEIGHT * getScale();  // position at bottom screen edge
...
{
	PlaySound("sound\\powerup.wav", NULL, SND_ASYNC);
	*power = true;
}
